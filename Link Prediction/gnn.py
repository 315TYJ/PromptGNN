...
def train(model, predictor, x, adj_t, split_edge, optimizer, batch_size,loader,device):
...
def main():
    ...
    if hasattr(data, 'adj_t'):
        data.edge_index = data.adj_t.to_dense().nonzero(as_tuple=False).t().contiguous()
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, emb.weight, adj_t, split_edge,
                         optimizer, args.batch_size,loader,device)
